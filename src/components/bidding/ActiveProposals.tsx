...
const mockProposals: Proposal[] = [
  {
    ...
    winProbability: 75,
    notes:
      "Client requested additional security documentation. Follow-up meeting scheduled.",
  },
  {
    id: "P002",
    tenderId: "T002",
    ...
    winProbability: 60,
    notes:
      "Proposal submitted on time. Competitive landscape with 5 other bidders.",
  },
  {
    id: "P003",
    tenderId: "T005",
  ...
  },
  {
    ...
    notes:
      "Lost to competitor with lower bid. Feedback: Price too high for scope.",
  },
];
...
export function ActiveProposals() {
  ...
  const filterProposals = (status: string) => {
    ...
    if (status === "active")
      return mockProposals.filter((p) =>
        ["submitted", "under-review", "shortlisted"].includes(p.status)
      );
  ...
  const totalValue = mockProposals
    ...
    .reduce(
      (sum, p) => sum + parseInt(p.proposedValue.replace(/[^0-9]/g, "")),
      0
    );

  const avgWinRate =
    mockProposals.filter((p) => ["won", "lost"].includes(p.status)).length > 0
      ? (mockProposals.filter((p) => p.status === "won").length /
          mockProposals.filter((p) => ["won", "lost"].includes(p.status))
            .length) *
        100
      : 0;
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
        ...
        <Card className="bg-gray-900/50 border-gray-800">
          <CardContent className="p-4 text-center">
            ...
            <p className="text-2xl text-white">{formatCurrency(totalValue)}</p>
                        ...
                        <div className="flex items-start justify-between mb-4">
                          <div className="flex-1">
                            <div className="flex items-start justify-between mb-2">
                              <h3 className="text-xl text-white pr-4">
                          ...
                          <div className="mb-4">
                            <div className="flex items-center justify-between mb-2">
                              ...
                              <span className="text-white text-sm">
                                {proposal.completionScore}%
                              </span>
                            </div>
                            <Progress
                              value={proposal.completionScore}
                              className="h-2"
                            />
                          </div>
                        )}
