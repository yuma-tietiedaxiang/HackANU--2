...
const budgetScenarios = [
  { name: 'Conservative', q1: 225000, q2: 240000, total: 920000, probability: 75 },
  { name: 'Realistic', q1: 250000, q2: 275000, total: 1050000, probability: 60 },
  { name: 'Optimistic', q1: 285000, q2: 320000, total: 1250000, probability: 25 }
];
...
const aiInsights = [
  {
    ...
    description: 'Marketing spend efficiency could improve by 15% with better targeting',
    impact: '$1,800',
  ...
  },
  {
    ...
    description: 'Travel expenses may exceed budget by 25% in Q1 due to conference season',
    impact: '+$2,200',
  ...
  },
  {
    ...
    description: 'SaaS costs typically spike 8% in January due to annual renewals',
    impact: '+$1,200',
    color: 'text-blue-400'
  }
];
...
export function Forecasting() {
  const totalForecast6Months = forecastData
    .filter(d => d.forecast)
    .reduce((sum, d) => sum + (d.forecast || 0), 0);

  const avgMonthlyForecast = Math.round(totalForecast6Months / 6);
  const currentMonth = 75000;
  const nextMonthForecast = 78500;
  const monthlyGrowth = ((nextMonthForecast - currentMonth) / currentMonth * 100).toFixed(1);
          ...
          <CardContent className="p-4">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-2xl text-white">${totalForecast6Months.toLocaleString()}</p>
              </div>
          ...
          <CardContent className="p-4">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-2xl text-white">${avgMonthlyForecast.toLocaleString()}</p>
              </div>
            ...
            <div className="flex items-center justify-between">
              <div>
                ...
                <div className="flex items-center space-x-2">
                  <p className="text-2xl text-white">${nextMonthForecast.toLocaleString()}</p>
                  <TrendingUp className="w-4 h-4 text-green-400" />
                </div>
                <p className="text-sm text-green-400">+{monthlyGrowth}%</p>
              </div>
              <Badge className="bg-green-500/20 text-green-400">85% Confidence</Badge>
            </div>
          </CardContent>
        </Card>
            ...
            {categoryForecasts.map((category, index) => (
              ...
              >
                <div className="flex items-center justify-between mb-3">
                  ...
                  <div className="flex items-center space-x-2">
                    <Badge 
                      variant={category.riskLevel === 'low' ? 'default' : category.riskLevel === 'medium' ? 'secondary' : 'destructive'}
                      className="text-xs"
                    >
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                  <div>
                    ...
                    <p className="text-xl text-white">${category.current.toLocaleString()}</p>
                  </div>
                  <div>
                    <p className="text-gray-400 text-sm">Forecasted</p>
                    <p className="text-xl text-white">${category.forecast.toLocaleString()}</p>
                    <p className="text-sm text-gray-400">
                      {((category.forecast - category.current) / category.current * 100).toFixed(1)}% change
                    </p>
                  </div>
                  <div>
                    <p className="text-gray-400 text-sm">Confidence</p>
                    <div className="flex items-center space-x-2">
                      <Progress value={category.confidence} className="flex-1 h-2" />
                      <span className="text-white text-sm">{category.confidence}%</span>
                    </div>
                  </div>
                </div>
                ...
                <div className="mt-3">
                  ...
                  <div className="flex flex-wrap gap-1 mt-1">
                    {category.factors.map((factor, factorIndex) => (
                      <Badge key={factorIndex} variant="outline" className="text-xs border-gray-600 text-gray-300">
                        {factor}
                      </Badge>
                    ))}
                  </div>
            ...
            {budgetScenarios.map((scenario, index) => (
              ...
              >
                <div className="flex items-center justify-between mb-3">
                  ...
                  <Badge className={`${
                    scenario.name === 'Realistic' ? 'bg-blue-500/20 text-blue-400' : 'bg-gray-600/20 text-gray-400'
                  }`}>
                ...
                <div className="space-y-3">
                  <div>
                    ...
                    <p className="text-xl text-white">${scenario.q1.toLocaleString()}</p>
                  </div>
                  <div>
                    <p className="text-gray-400 text-sm">Q2 Projection</p>
                    <p className="text-xl text-white">${scenario.q2.toLocaleString()}</p>
                  </div>
                  <div>
                    <p className="text-gray-400 text-sm">Annual Total</p>
                    <p className="text-2xl text-white">${scenario.total.toLocaleString()}</p>
                  </div>
                </div>
              </motion.div>
            ))}
